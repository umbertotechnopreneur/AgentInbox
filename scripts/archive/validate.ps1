...
try {
    ...
    python (Join-Path $PSScriptRoot 'smoke-test.py') src/MailMeUp.Cli/bin/Release/net10.0/agentinbox.dll
    if ($LASTEXITCODE -ne 0) { throw 'Protocol smoke test failed.' }
    python (Join-Path $PSScriptRoot 'export-notices.py') --check
    if ($LASTEXITCODE -ne 0) { throw 'Dependency inventory is out of date.' }
    python (Join-Path $PSScriptRoot 'repo-check.py')
