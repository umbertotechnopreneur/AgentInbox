...
Each release contains complete Windows x64 and ARM64 portable ZIPs. Both include the native setup app, a separate CLI/MCP executable and their runtimes. A signed Windows x64 MSIX is optional. See [portable setup](WINDOWS_PORTABLE.md). The workflow does not publish Linux or macOS archives.

Only Windows x64 is tested for the current MVP. Windows ARM64 is build-only.

Each package includes the application, .NET runtime, license notices, version information and a SHA-256 checksum; ZIPs also record the source commit. When an MSIX is attached, include its public certificate for devices that must trust its signing identity.
...
Run **Actions > Windows release > Run workflow** on `main`. It builds the portable ZIPs and, when signing is configured, the MSIX as workflow artifacts, without creating a release.
...
Dispatch never publishes a release. A matching tag creates a pre-release draft for review, or updates the assets of an existing draft while preserving its notes and locally uploaded installer. Published releases are never overwritten. Publish manually after reviewing the assets. A tag must match the project version and point to code reachable from `main`.
...
Automated MSIX signing uses these repository secrets: `MSIX_CERTIFICATE_BASE64` (the Base64-encoded PFX), `MSIX_CERTIFICATE_PASSWORD`, `MSIX_PUBLISHER` (the certificate subject, for example `CN=umber`), `MSIX_PUBLISHER_DISPLAY_NAME`, and `MSIX_TIMESTAMP_SERVER`. If none are configured, the workflow skips MSIX signing and still produces the portable release. A partial configuration fails with the missing secret names.

For automated signing, the workflow imports the PFX into the ephemeral runner's CurrentUser store, signs the package, then removes the imported certificate and temporary PFX. Alternatively, build and sign locally with `scripts/package-msix.ps1`, then attach the `.msix`, public `.cer` and checksum to the draft. Keep the private key out of the repository and release assets.
