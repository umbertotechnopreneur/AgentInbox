...
2. **Connect accounts:** choose Google or Microsoft and finish sign-in in the browser. Repeat to add more accounts. **Check connections** verifies every locally connected account with minimal Mail/Calendar API requests, silently renewing a token where the provider allows it. It shows only a safe status; reconnect accounts marked **Reconnect needed**. This preview still requires [your own provider app registration](APP_REGISTRATION.md); Google imports the downloaded Desktop client JSON, and Microsoft accepts the Application (client) ID.
